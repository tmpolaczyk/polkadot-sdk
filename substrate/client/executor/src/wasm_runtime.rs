...
impl RuntimeCache {
	...
	pub fn with_instance<'c, H, R, F>(
		...
		wasm_method: WasmExecutionMethod,
		wasmtime_precompiled: Option<&Path>,
		heap_alloc_strategy: HeapAllocStrategy,
...
pub fn create_wasm_runtime_with_code<H>(
	...
	wasmtime_precompiled_path: Option<&Path>,
	code_hash: &[u8],
...
pub fn precompile_and_serialize_versioned_wasm_runtime<'c>(
	...
	let blob = sc_executor_common::runtime_blob::RuntimeBlob::uncompress_if_needed(code.as_ref())?;
...
/// Compute a hash that aggregates all the metadata relating to the artifact that must not change
/// so that it can be reused with confidence.
fn compute_artifact_version(
	allow_missing_func_imports: bool,
	code_hash: &[u8],
	...
	buffer.push(allow_missing_func_imports as u8);
	semantics.encode_to(&mut buffer);

	let hash = sp_core::hashing::blake2_256(&buffer);
	sp_core::bytes::to_hex(&hash, false)
}
